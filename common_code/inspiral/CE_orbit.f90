...
! ***********************************************************************
      subroutine CE_orbit_adjust(id, ierr)
         ...
         integer :: k
         real(dp) :: CE_energy_rate, CE_companion_position, CE_companion_mass, CE_drag_work
         ...
         real(dp) :: value, magv2
         real(dp) :: G, semimax, eangmom_ini, eenergy_ini
         integer :: index
         ...
         if (s% x_logical_ctrl(7)) then
            ...
            if (s% model_number .eq. 1) then
               ...
               if (s% x_logical_ctrl(8)) then
                  ...
                  G = 392512559.8496094d0
                  s% xtra(2) = s% x_ctrl(2) * s% r(1) / Rsun
                  s% xtra(25) = 0.d0         ! nu (rad)
                  !s% xtra(26) = s% x_ctrl(20) ! dr/dt (Rsun/yr)
                  !s% xtra(27) = s% x_ctrl(21) ! dnu/dt (rad/yr)
                  semimax = s% xtra(2) / (1.0 + s% x_ctrl(21)) ! r / (1 +  eccentricity)
                  eangmom_ini = sqrt( semimax * (1.d0 - s% x_ctrl(21) * s% x_ctrl(21)) * G * (M_encl / Msun + CE_companion_mass) )
                  eenergy_ini = - G * (M_encl / Msun + CE_companion_mass) / (2.d0 * semimax)

                  s% xtra(27) = eangmom_ini / (s% xtra(2) * s% xtra(2)) ! dnu/dt (rad/yr)
                  if (s% xtra(25).eq.0.d0 .or. s% xtra(25).eq.pi) then
                     s% xtra(26) = 0.d0  ! dr/dt (Rsun/yr)
                  else
                     s% xtra(26) = - sqrt( 2.d0 * (eenergy_ini + G * (M_encl / Msun + CE_companion_mass) / s% xtra(2)) -  eangmom_ini * eangmom_ini / (s% xtra(2) * s% xtra(2))) ! dr/dt (Rsun/yr)
                  endif
               ...
               write(*,*) "!!!!!!!!!!!!!!!!!!!!!!!!!!!! Dynamic initial condition !!!!!!!!!!!!!!!!!!!!!!!!!!!!"
               write(*,*) "r (Rsun): ", s% xtra(2), " dr/dt (Rsun/yr): ", s% xtra(26)
               write(*,*) "nu (rad): ", s% xtra(25), " dnu/dt (rad/yr): ", s% xtra(27)
            end if 
            ...
            !orbital_ang_mom_lost = (s% xtra(2) * Rsun ) * (- abs(s% xtra(30)) * Msun * Rsun / (secyer*secyer)) * sin(atan2(s% xtra(27)) - )
            ...
            magv2 = (s% xtra(26)*Rsun/secyer)*(s% xtra(26)*Rsun/secyer) + (s% xtra(2) * Rsun)*(s% xtra(2) * Rsun)*(s% xtra(27) / secyer)*(s% xtra(27) / secyer)
            E_final = (magv2/2.0 - standard_cgrav * (CE_companion_mass * Msun + M_final) / (s% xtra(2) * Rsun) ) * (CE_companion_mass * Msun * M_final) / (CE_companion_mass * Msun + M_final) 
            
            ! CE energy rate
            CE_drag_work = s% xtra(11)  * Msun * Rsun*Rsun / (secyer*secyer) ! work done by dragforce in [g * cm^2 / s^2]
            s% xtra(1) = - (CE_drag_work) / s% dt  !- (E_final - E_init) / s% dt
         ...
         write(*,*) "Change in Orbital Energy = ", - s% xtra(1) * s% dt
         ...
         if (s% lxtra(1)) then
            ...
            s% P_function_weight = 0.0d0
            s% T_function1_weight = 0.0d0
            s% R_function_weight = 500.0d0
            s% R_function_param = 1.0d0
         endif
...
! **********************************************************************
      subroutine orbit_integration(id,ierr)
         ...
         real(8),parameter              :: tol   = 1.0d-15       !! integration tolerance
         real(8),parameter              :: r_tol   = 1.0d-13       !! integration tolerance relative to the answer
         real(8),parameter              :: a_tol   = 1.0d-15       !! absolute integration tolerance
         ...
         integer :: step, Nstep
         real(8) :: dx = 1.d-3  , xf , x0, distance, work, dwork
         real(8) :: r0, theta0
         ...
         if (s% x_integer_ctrl(4) <= 0 ) then
            ...
            !back up coordinates before the solver iteration
            r0 = y(1)
            theta0 = y(3)
            ...
            distance = sqrt( r0*r0 + y(1)*y(1) - 2.0d0 * r0 * y(1) * cos(theta0 - y(3)) )
            work = distance * s% xtra(30)
         else 

            ! iterate over the x_integer_ctrl(4) steps 
            dx = (x +  s% dt)/secyer / s% x_integer_ctrl(4)

            work = 0.0d0
            do step = 1, s% x_integer_ctrl(4) , 1
               xf = x0 + dx * step
               ...
               ! compute the work done by the drag force in the current iteration [Msun * Rsun^2 / yr^2]
               dwork = distance * s% xtra(30)
               work = work + dwork

               ! obtain the radial and angular accelerations (useful for computing GS signals from CEE events)
               call kepler_polar(prop,x0 + dx * (step - 1), y, f)
         ...
         ! Work done by the drag force (Msun * Rsun^2 / yr^2)
         s% xtra(11)  = - abs(work)   
   ...
   !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      ...
      subroutine kepler_polar(me,x,y,f)
         ...
         ! acretion radius (cm)
         R_acc = s% xtra(12)
         !write(*,*) "*************** Acc. radius in solver (R_sun) = ", R_acc / Rsun
         ...
         if (r.gt.renv) then
            ...
            if (s% x_integer_ctrl(2) .eq. 4) then
               ...
               tmp2 =  pi   * tmprho * (R_acc / Rsun)*(R_acc / Rsun) * (magv*magv) / tmp1
               ...
               Fext = - (tmp2 * tmp3 ) ! this should be in units of acceleration
               !Fext = 0.d0
